...
import { Download, RefreshCcw, Search, Network, Cloud, Database, Cpu, Lock, Maximize2, Server, Layers, Video, ShieldAlert, Workflow, Zap, X, Activity, ArrowRight, Clock, HardDrive, BarChart3, GitBranch, Globe } from "lucide-react";
...
const nodes: NodeData[] = [
  {
    ...
    color: "text-zinc-300", bg: "bg-zinc-800/10", gradient: "from-zinc-800 to-zinc-950",
  ...
  {
    id: "aicoo", label: "Aicoo Network", subtitle: "Cross-Org Ledger", icon: Globe,
    color: "text-zinc-300", bg: "bg-zinc-800/10", gradient: "from-zinc-800 to-zinc-950",
    x: 500, y: 30, w: 180, status: "healthy",
    metrics: [{ label: "Peers", value: "1,204" }, { label: "Blocks/s", value: "45" }, { label: "Latency", value: "8ms" }],
    description: "Decentralized cryptographic ledger for autonomous agent-to-agent negotiations and secure cross-organizational data sharing.",
    connections: ["orchestrator"], tech: ["Ed25519", "WebSockets", "Raft Consensus", "Zero-Trust"],
  },
];
...
const connectionPaths: { from: string; to: string; active: boolean; path: string }[] = [
  ...
  { from: "aicoo", to: "orchestrator", active: true, path: "M 590,120 C 590,160 590,160 590,200" },
...
export default function ArchitecturePage() {
  ...
  return (
    <div className="px-4 md:px-8 pb-8 max-w-[1600px] mx-auto min-h-[calc(100vh-4rem)] flex flex-col pt-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-4 flex justify-between items-end shrink-0">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={handleExportConfig} className="bg-gradient-to-r from-zinc-800 to-zinc-950 text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:shadow-lg hover:shadow-black/25 transition-all flex items-center gap-2 active:scale-[0.98]">
            <Download className="w-4 h-4" /> Export Config
          </button>
        </div>
      </motion.div>

      <div className="w-full h-[600px] flex gap-4 overflow-hidden mb-8">
        {/* Canvas */}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="h-10 border-b border-outline-variant/30 bg-surface-container/30 backdrop-blur-md flex items-center justify-between px-4 z-10 shrink-0">
            <div className="flex items-center gap-2 text-xs font-medium text-on-surface-variant">
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-zinc-800 animate-pulse shadow-[0_0_4px_rgba(16,185,129,0.5)]" /> Production (US-East)</span>
              <span className="mx-2 text-outline-variant/50">|</span>
              <span className="flex items-center gap-1"><Zap className="w-3 h-3 text-zinc-300" /> Auto-Scaling</span>
              <span className="mx-2 text-outline-variant/50">|</span>
              <span className="flex items-center gap-1"><Activity className="w-3 h-3 text-zinc-300" /> 10 Services</span>
            </div>
            <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-wider text-on-surface-variant/40">
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-zinc-800" /> Healthy</span>
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-zinc-800 animate-pulse" /> Active</span>
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-zinc-800" /> Warning</span>
            </div>
          </div>
            ...
            <div className="min-w-[1500px] h-[560px] relative p-4">
              ...
              <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }}>
                <defs>
                  ...
                  <marker id="ah-active" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto"><polygon points="0 0,8 3,0 6" fill="#a1a1aa" /></marker>
                  <marker id="ah-hl" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto"><polygon points="0 0,8 3,0 6" fill="#d4d4d8" /></marker>
                </defs>
                {connectionPaths.map((c, i) => {
                  const hl = isPathHighlighted(c.from, c.to);
                  return (
                    <path key={i} d={c.path} fill="none"
                      stroke={hl ? "#d4d4d8" : c.active ? "#a1a1aa" : "var(--outline-color)"}
                      strokeWidth={hl ? 3 : 2}
              ...
              {/* Security wrapper */}
              <div className="absolute left-[765px] top-[350px] w-[200px] h-[120px] border border-dashed border-zinc-600/30 rounded-xl pointer-events-none" style={{ zIndex: 1 }}>
                <div className="absolute -top-3 left-3 bg-surface-container-lowest/80 px-1.5 flex items-center gap-1 text-[9px] text-zinc-300 font-bold tracking-wider uppercase"><Lock className="w-3 h-3" /> Model Armor</div>
              </div>
              {/* Inference group */}
              <div className="absolute left-[1285px] top-[120px] w-[200px] h-[210px] border border-dashed border-zinc-600/25 rounded-xl pointer-events-none" style={{ zIndex: 1 }}>
                <div className="absolute -top-3 right-3 bg-surface-container-lowest/80 px-1.5 flex items-center gap-1 text-[9px] text-zinc-300 font-bold tracking-wider uppercase">Inference Pipeline</div>
              </div>
                ...
                return (
                  ...
                  >
                    <div className={`bg-surface/90 backdrop-blur-sm rounded-xl p-3.5 border-2 transition-all duration-300 ${
                      isSelected ? "border-zinc-400 shadow-[0_0_20px_rgba(161,161,170,0.2)] scale-105" :
                      connected ? "border-zinc-500/30 shadow-md" :
                      node.status === "active" ? "border-zinc-600/40 shadow-[0_0_12px_rgba(212,212,216,0.1)]" :
                      "border-outline-variant/50 hover:border-outline-variant shadow-sm hover:shadow-md"
                    }`}>
                      {node.status === "active" && !isSelected && <div className="absolute inset-0 rounded-xl bg-zinc-800/5 animate-pulse pointer-events-none" />}
                      <div className="flex items-center gap-2.5 mb-2 relative z-10">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            ...
                            <span className={`w-2 h-2 rounded-full shrink-0 ml-1 ${node.status === "active" ? "bg-zinc-800 animate-pulse shadow-[0_0_4px_rgba(16,185,129,0.5)]" : node.status === "warning" ? "bg-zinc-800" : "bg-zinc-800"}`} />
                          </div>
                      ...
                      {node.id === "orchestrator" && (
                        <div className="relative z-10 mt-1">
                          <div className="w-full h-1.5 bg-surface-container-high/50 rounded-full overflow-hidden">
                            <motion.div initial={{ width: 0 }} animate={{ width: "67%" }} transition={{ duration: 1.5 }} className="h-full bg-gradient-to-r from-zinc-800 to-zinc-950 rounded-full" />
                          </div>
                      ...
                      {node.id === "gpu" && (
                        <div className="flex items-center gap-1 mt-1.5 relative z-10">
                          {[true,true,true,false].map((on, i) => <span key={i} className={`w-2 h-2 rounded-full ${on ? "bg-zinc-800 animate-pulse" : "bg-surface-container-high"}`} style={on ? { animationDelay: `${i*200}ms` } : {}} />)}
                          <span className="text-[9px] text-on-surface-variant/50 ml-1.5">3/4 Active</span>
                        </div>
                      )}
                      {node.id === "queue" && (
                        <div className="flex justify-between text-[9px] text-on-surface-variant/50 mt-1.5 relative z-10"><span>Pending: 24</span><span className="text-zinc-300 font-semibold">Processing</span></div>
                      )}
                    </div>
                  </motion.div>
                );
            ...
            <motion.div key={selected.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.25 }}
              ...
              <div className="p-5 border-b border-outline-variant/30">
                ...
                <div className={`inline-flex items-center gap-1.5 mt-3 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border ${
                  selected.status === "active" ? "bg-zinc-800/8 text-zinc-300 border-zinc-600/15" :
                  selected.status === "warning" ? "bg-zinc-800/8 text-zinc-300 border-zinc-600/15" :
                  "bg-zinc-800/8 text-zinc-300 border-zinc-600/15"
                }`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${selected.status === "active" ? "bg-zinc-800 animate-pulse" : "bg-zinc-800"}`} />
      ...
      <div className="mt-4 shrink-0 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="premium-card rounded-xl p-5 flex items-start gap-4">
          ...
          <div>
            <h4 className="text-sm font-bold text-on-surface">Event-Driven Telemetry</h4>
            <p className="text-[11px] text-on-surface-variant/80 mt-1.5 leading-relaxed">
              The entire macro-architecture is fully observable. Request tracing propagates from the Edge WAF, through the Next.js API Gateway, down to the asynchronous GPU clusters, enabling microsecond-precision debugging.
            </p>
          </div>
        </div>
        <div className="premium-card rounded-xl p-5 flex items-start gap-4">
          ...
          <div>
            <h4 className="text-sm font-bold text-on-surface">Aicoo Ledger Sync</h4>
            <p className="text-[11px] text-on-surface-variant/80 mt-1.5 leading-relaxed">
              The MCP Orchestrator maintains a persistent WebSocket connection to the Aicoo Network. This ensures that B2B negotiations established by executive agents are cryptographically verified and immutable.
            </p>
          </div>
        </div>
        <div className="premium-card rounded-xl p-5 flex items-start gap-4 md:col-span-2 lg:col-span-1">
          ...
          <div>
            <h4 className="text-sm font-bold text-on-surface">Dynamic Scalability</h4>
            <p className="text-[11px] text-on-surface-variant/80 mt-1.5 leading-relaxed">
              The system relies on AWS EKS auto-scaling coupled with Redis Bull Queue to dynamically provision A100 GPU spot instances based on the orchestrator's inference payload depth.
            </p>
          </div>
        </div>
      </div>
